...
SmoothingScheme::SmoothingScheme(
    ...
    std::vector<std::vector<int>> connectivity_tmp(connectivity.size(),
                                                   std::vector<int>(4));
    std::vector<std::vector<double>> coordinates_tmp(coordinates.size(),
                                                     std::vector<double>(3));
...
void SmoothingScheme::executeSmoothing(
...
std::vector<double> SmoothingScheme::calculateMovingDirection(int _node_id) {
    ...
    for (int inode : adj_nodes) {
        ...
        for (int idim = 0; idim < 3; ++idim) {
            moving_direction[idim] +=
                (m_coordinates[inode][idim] - m_coordinates[_node_id][idim]);
        }
        num_adj_nodes += 1;
    }
...
void SmoothingScheme::moveNodes() {
    std::vector<std::vector<double>> new_coordinates(m_coordinates.size(),
                                                     std::vector<double>(3));
    std::copy(m_coordinates.begin(), m_coordinates.end(),
...
double SmoothingScheme::findMaxAspectRatio() {
    ...
    for (int ielem = 0; ielem < num_elem_smooth; ++ielem) {
        ...
        for (int inode = 0; inode < 4; ++inode) {
            prev_verts[inode] =
                m_coordinates_prev[m_connectivity[elem_id][inode]];
        }
