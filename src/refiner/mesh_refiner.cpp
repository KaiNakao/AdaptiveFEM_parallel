...
void Refiner::executeRefinement()
{
    ...
    refinement_scheme.executeRefinement(new_connectivity, new_coordinates);
    // 1. DO node movement
    // 2. DO mesh refinement

    // output new mesh
    std::ofstream ofs;
    // ofs.open("new_coordinates.bin", std::ios::binary);
    ...
    // ofs.open("new_connectivity.bin", std::ios::binary);
