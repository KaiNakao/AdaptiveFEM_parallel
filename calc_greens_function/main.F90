program main
    ...
    double precision :: uvec(24), xnode(3, 8), xc(3), mvec(6), &
                        fvec(24)
    double precision, allocatable :: gf(:)
    integer :: nobs, nload, iload

    call read_nload(nload)
    allocate(gf(nload))

    call read_nobs_xnode(nobs, xnode)

    call read_centroid(xc, mvec) 

    call calc_fvec(fvec, xnode, xc, mvec)
    print *, "fvec:", fvec

    do iload = 1, nload
        call read_displacement(uvec, iload)

        gf(iload) = dot_product(fvec, uvec)
    end do
    call output_gf(gf)
...
contains
    ...
    subroutine read_displacement(uvec, iload)
        implicit none
