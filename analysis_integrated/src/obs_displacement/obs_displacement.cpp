#include "obs_displacement.hpp"

Obs_Displacement::Obs_Displacement(int myid, int numprocs) {
    // constructor
    this->myid = myid;
    this->numprocs = numprocs;
}

void Obs_Displacement::prep() {
    // preparation steps
    int nelem, nnode, nmaterial, nload;
    nsp_obs_displacement::read_shape(myid, nelem, nnode, nmaterial, nload);

    std::vector<int> matid_arr;
    nsp_obs_displacement::read_mesh(myid, nelem, nnode, cny, coor, matid_arr);
    ...
    nload = 1;
#else
    nsp_obs_displacement::read_obs_point(myid, obs_point);
#endif
    ...
    unode.resize(10, std::vector<double>(3));
    nvec.resize(10); u.resize(3); u_arr_buf.resize(3 * nfound);
}

void Obs_Displacement::exec(int iload, const std::vector<double> &uvec) {
    std::vector<std::vector<double>> displacement(coor.size(),
                                                    std::vector<double>(3));
    for (int inode = 0; inode < coor.size(); inode++) {
        for (int idim = 0; idim < 3; idim++) {
            displacement.at(inode).at(idim) = uvec.at(inode * 3 + idim);
        }
    }
    ...
    if (myid == 0) {
        ...
        nsp_obs_displacement::write_displacement_obs(result, iload);
    }
}
