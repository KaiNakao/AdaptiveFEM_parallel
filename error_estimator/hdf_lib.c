...
static herr_t my_H5LT_make_dataset_numerical(hid_t loc_id,
                                             const char* dset_name, int rank,
                                             const hsize_t* dims, hid_t tid,
                                             const void* data) {
    hid_t did = -1, sid = -1;
    ...
    /* Create the dataset. */
    if ((did = H5Dcreate2(loc_id, dset_name, tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
        goto out;
...
static herr_t my_H5LT_read_dataset_numerical(hid_t loc_id,
                                             const char* dset_name, hid_t tid,
                                             void* data) {
    hid_t did;

    /* Open the dataset. */
    if ((did = H5Dopen2(loc_id, dset_name, H5P_DEFAULT)) < 0) return -1;

    /* Read */
    if (H5Dread(did, tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) < 0) goto out;

    /* End access to the dataset and release resources used by it. */
    if (H5Dclose(did)) return -1;
...
void set_null_char_(char* filename, long* pos) { filename[*pos] = '\0'; }
...
void hdf_write_double_array_(long* fid, char* type, double* array, long* size) {
    ...
    dims[0] = *size;
    rank = 1;
    status = my_H5LT_make_dataset_numerical(glb_file_id[*fid], type, rank, dims,
                                            R64, array);
    t = omp_get_wtime() - t;
#ifdef PRTDBG
...
/* read double array */
void hdf_read_double_array_(long* fid, char* type, double* array, long* memsize,
                            long* size) {
    double t;
    hsize_t dims[1];
    herr_t status;
    ...
    *size = dims[0];
    ...
    status =
        my_H5LT_read_dataset_numerical(glb_file_id[*fid], type, R64, array);
    t = omp_get_wtime() - t;
#ifdef PRTDBG
...
/* read long array */
void hdf_read_long_array_(long* fid, char* type, long* array, long* memsize,
                          long* size) {
    double t;
    hsize_t dims[1];
    herr_t status;
    ...
    *size = dims[0];
    ...
    status =
        my_H5LT_read_dataset_numerical(glb_file_id[*fid], type, I64, array);
    t = omp_get_wtime() - t;
#ifdef PRTDBG
    printf(" read %s took (s): %f\n", type, t);
    printf(" status ld %ld\n", status);
...
/* read int array */
void hdf_read_int_array_(long* fid, char* type, int* array, long* memsize,
                         long* size) {
    double t;
    hsize_t dims[1];
    herr_t status;
    ...
    *size = dims[0];
    ...
    status =
        my_H5LT_read_dataset_numerical(glb_file_id[*fid], type, I32, array);
    t = omp_get_wtime() - t;
#ifdef PRTDBG
    printf(" read %s took (s): %f\n", type, t);
    printf(" status ld %ld\n", status);
...
void hdf_read_long_array_part_(long* fid, char* type, long* start, long* size,
                               long* val) {
    ...
    status =
        H5Dread(dataset_id, I64, memspace_id, dataspace_id, H5P_DEFAULT, val);
...
void hdf_read_int_array_part_(long* fid, char* type, long* start, long* size,
                              int* val) {
    hsize_t dims[1], dimsm[1];
    hid_t dataset_id;
    hid_t dataspace_id, memspace_id;
    ...
    status =
        H5Dread(dataset_id, I32, memspace_id, dataspace_id, H5P_DEFAULT, val);
    status = H5Sclose(memspace_id);
    status = H5Sclose(dataspace_id);
    status = H5Dclose(dataset_id);
}
