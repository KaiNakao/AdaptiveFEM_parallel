...
//    glb_file_id[*fid] = H5Fopen(filename, H5F_ACC_RDWR, H5P_DEFAULT);
    glb_file_id[*fid] = H5Fopen(filename, H5F_ACC_RDONLY, H5P_DEFAULT);
}
...
/* get long array component */
void hdf_read_double_array_part_( long* fid, char* type, long* start, long* size, double* val )
{
   hsize_t dims[1], dimsm[1];
   hid_t dataset_id;
   hid_t       dataspace_id, memspace_id;
   ...
   status = H5Dread (dataset_id, R64, memspace_id, dataspace_id, H5P_DEFAULT, val);
   status = H5Sclose (memspace_id);
   status = H5Sclose (dataspace_id);
   status = H5Dclose (dataset_id);
}
